...
$sql = "SELECT * FROM banco_bacteriologas WHERE tipo='Auxiliar' ORDER BY nombre;";
...
if ($users = $db->get_results($sql, ARRAY_A)) {
    ...
    foreach ($users as $user) {
        $i++;
        $vNomEnfermera = htmlentities($user['nombre'], ENT_COMPAT);

        $cadbact.=" { ";
        $cadbact.=$comilla2."codigo".$comilla2." : ".$comilla2.$user['codigo'].$comilla2;
        $cadbact.=$c.$comilla2."nombre".$comilla2.": ".$comilla2.$vNomEnfermera.$comilla2;
