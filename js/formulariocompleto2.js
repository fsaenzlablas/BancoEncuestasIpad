...
var vMostrarEFisico = 1; //Mostrar campos del examen fisico
...
//                        alert($componenteStr);
                ...
                $('#popRespComponentes :selected').each(function(i, selected){
                    ...
                    //                  alert("sseleccionados "+$(selected).val());
                });
                for(var $k=0;$k<vArrComponentesSeleccionados.length;$k++){
//                    alert("CON EL ARREGLO  "+vArrComponentesSeleccionados[$k]);
...
//alert($valor);
                        $cadNomComponentes += "<tr><td>"+$(selected).text()+"</td><td>"+$(selected).val()+"</td></tr>";
...
//                        alert("tambien en "+$valor);
                        ...
                        $cadNomComponentes += "<tr><td>"+$(selected).text()+"</td><td>"+$(selected).val()+"</td></tr>";
                        /*
                         if ($cadTipo.indexof($valor+"\" selected")>=0){//ya estaba seleccionado ya no hacer nada
                         $cadNomComponentes += "<tr><td>"+$(selected).text()+"</td><td>"+$(selected).val()+"</td></tr>";
                         $cadTipo=$cadTipo.replace("value=\""+$valor+"\" selected","value=\""+$valor+"\"");
                         $cadTipo=$cadTipo.replace("value=\""+$valor+"\"","value=\""+$valor+"\" selected");

                         }else{//si no estaba marcado tampoco hacer nada
                         $cadTipo=$cadTipo.replace("value=\""+$valor+"\" selected","value=\""+$valor+"\"");
...
function ocultarExamenFisico(){
//    alert("ocultando");
//    $('#idExamenFisico').hide();
    vMostrarEFisico = 1-vMostrarEFisico;//prender o apagar.
    var rows = $('table.Demograficos tr');
    var eFisico = rows.filter('.eFisico');
    if (vMostrarEFisico==0){
        eFisico.hide();
        $('#bExamenFisico').value="Mostrar E.Fisico";

    }
    else{
        eFisico.show();
        $('#bExamenFisico').value="Ocultar E.Fisico";
...
function imprimirCodBarras(){
    var codBarra;
    if ($('#txtOrden').val()==""){
        codBarra = prompt("Número de Donación:","");

    }else {
        codBarra = prompt("Número de Donación:",$('#txtOrden').val());
...
function showDialog()
{
    ...
    if ($('#txtOrden').val()!= ""){
        ...
        }else alert("No se encontro informacion de la encuesta")

    }else{
        alert("Falta Seleccionar un numero de encuesta")
    }
